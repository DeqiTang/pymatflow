...
if __name__ == "__main__":
    ...
    task.converge_rel_cutoff(emin=args.range[0], emax=args.range[1], step=args.range[2], directory=args.directory, cutoff=args.cutoff, runopt=args.runopt, jobname=args.jobname, nodes=args.nodes, ppn=args.ppn)
    ...
    if args.auto == 0:
        ...
        if args.server == "pbs":
            ...
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="converge-relcutoff.pbs", server="pbs")
        elif args.server == "yh":
            ctl.get_info(os.path.join(os.path.expanduser('~'), ".pymatflow/server_yh.conf"))
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="converge-rel-cutoff.inp.sub", server="yh")
