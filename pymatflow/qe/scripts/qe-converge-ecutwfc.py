...
if __name__ == "__main__":
    ...
    task.converge_ecutwfc(args.range[0], args.range[1], args.range[2], directory=args.directory, runopt=args.runopt)   
    ...
    if args.auto == 0:
        ...
        if args.server == "pbs":
            ...
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="converge-ecutwfc.pbs", server="pbs")
        elif args.server == "yh":
            ctl.get_info(os.path.join(os.path.expanduser('~'), ".pymatflow/server_yh.conf"))
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="converge-ecutwfc.sub", server="yh")
