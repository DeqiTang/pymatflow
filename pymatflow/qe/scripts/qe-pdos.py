...
if __name__ == "__main__":
    ...
    parser.add("--jobname", type=str, default="qe-projwfc-pdos",
    ...
    if args.auto == 0:
        ...
        if args.server == "pbs":
            ...
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="static-projwfc.pbs", server="pbs")
        elif args.server == "yh":
            ctl.get_info(os.path.join(os.path.expanduser('~'), ".pymatflow/server_yh.conf"))
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="static-projwfc.sub", server="yh")
