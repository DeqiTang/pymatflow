...
if __name__ == "__main__":
    ...
    task.pp(directory=args.directory, runopt=args.runopt, jobname=args.jobname, nodes=args.nodes, ppn=args.ppn)
    ...
    if args.auto == 0:
        ...
        if args.server == "pbs":
            ...
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="pp.x.pbs", server="pbs")
        elif args.server == "yh":
            ctl.get_info(os.path.join(os.path.expanduser('~'), ".pymatflow/server_yh.conf"))
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="pp.x.sub", server="yh")
