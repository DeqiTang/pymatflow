...
if __name__ == "__main__":
    ...
    if args.auto == 0:
        ...
        if args.server == "pbs":
            ...
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="geometric-optimization.pbs", server="pbs")
        elif args.server == "yh":
            ctl.get_info(os.path.join(os.path.expanduser('~'), ".pymatflow/server_yh.conf"))
            ctl.login()
            ctl.submit(workdir=args.directory, jobfile="geometric-optimization.sub", server="yh")
