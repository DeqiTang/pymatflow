...
def main():
    ...
    os.mkdir(os.path.join(args.directory, "%2d" % (0)))
    write_structure(structure=initial, filepath=os.path.join(args.directory, "%2d/POSCAR" % (0)))
    os.mkdir(os.path.join(args.directory, "%2d" % (args.nimage+1)))
    write_structure(structure=initial, filepath=os.path.join(args.directory, "%2d/POSCAR" % (args.nimage+1)))
        
    for i in range(len(inter_images)):
        os.mkdir(os.path.join(args.directory, "%2d" % (i+1)))
        write_structure(structure=inter_images[i], filepath=os.path.join(args.directory, "%2d/POSCAR" % (i+1)))
