...
class siesta_properties:
    ...
    def to_fdf_band(self, fout, option=1):
        """
        option:
            0: using BandLines to specify kpoints for band calculation
            1: using BandPoints to specify kpoints for band calculation
        self.bandlines:
            a list of strings in format like below:
            ["1 0.000 0.000 0.000 \Gamma", ..., "20 1.0 1.0 1.0 L"]
        self. bandpoints:
        ...
        if option == 0:
            ...
            for line in self.bandlines:
                fout.write("%d %.3f %.3f %.3f %s\n" % (int(line.split()[0], float(line.split()[1]), float(line.split()[2]), float(line.split()[3]), line.split()[4])))
